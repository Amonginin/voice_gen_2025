...
from .score import asr_transcribe_text, composite_score, dummy_antispoof_score, speaker_similarity
...
def run_pipeline(cfg_path: str = "configs/pipeline.yaml") -> None:
    ...
    for idx, row in tqdm(tasks.iterrows(), total=len(tasks)):
        ...
        for c in cand_paths:
            post_wav = (tmp_dir / f"utt{utt}_post_{Path(c).stem}.wav").as_posix()
            ok = process_file(
                in_wav=c,
                out_wav=post_wav,
                sr=sr,
                lufs_target=post_cfg.get("lufs_target", -22.0) if post_cfg.get("enabled", True) else None,
                add_noise_dbfs=post_cfg.get("add_noise_dbfs", -35.0) if post_cfg.get("enabled", True) else None,
                highpass_hz=post_cfg.get("highpass_hz", 150) if post_cfg.get("enabled", True) else None,
                lowpass_hz=post_cfg.get("lowpass_hz", 8000) if post_cfg.get("enabled", True) else None,
            )
            if not ok:
                continue

            # score components
            hyp_text = asr_transcribe_text(post_wav, language=score_cfg.get("asr", {}).get("language", "zh"), model_size=score_cfg.get("asr", {}).get("model_size", "small")) if score_cfg.get("asr", {}).get("enabled", True) else ""
            spk_sim = speaker_similarity(ref_path, post_wav) if score_cfg.get("speaker", {}).get("enabled", True) else 0.0
            spoof_score = dummy_antispoof_score(post_wav) if score_cfg.get("antispoof", {}).get("enabled", False) else 0.5
    ...
    if export_cfg.get("make_zip", True):
        ...
        shutil.make_archive(base_name=zip_name.replace(".zip", ""), format="zip", root_dir=output_dir)
